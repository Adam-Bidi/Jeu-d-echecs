...
class Piece:
    ...
    def deplacement_autorise(self, case_souhaitee):
        def vect_unit(a, b, orientation = None):
            c = Position(*b)
            if orientation == "horizontal":
                a = Case((a.x, 50))
            if orientation == "vertical":
                a = Case((50, a.y))
            return Position((a.x - c.x) / abs(a.x - c.x), (a.y - c.y) / abs(a.y - c.y))
        ...
        if self.name == "pawn":
            ...
            def avancer_2_pion(case_souhaitee):
                ...
                p = int(p_c[h][k])
                if dico_n_c[p] == "white":
                    condition = str(Position(*self.position) + Position(0, -1))
                else : 
                    condition = str(Position(*self.position) + Position(0, 2))
                if (
                    str(pos) == condition
                    and bool(self.case_autorisee(case_souhaitee))
                    and bool(
            ...
            def manger_pion(case_souhaitee):
                ...
                if dico_n_c[p] == "white":
                    ...
                    bas_droit = str(Position(*self.position) + Position(1, -1))
                    bas_gauche = str(Position(*self.position) + Position(-1, -1))
                    return (str(pos) in {bas_droit, bas_gauche}) and bool(
                    self.case_autorisee(case_souhaitee)
                )
        ...
        if self.name == "bishop":
            ...
            if str(pos) in diagonals and bool(self.case_autorisee(case_souhaitee)):
                ...
                for k in range(1, abs(pos.x - Position(*self.position).x)):
                    n, m = (
                        int((Position(*self.position) + direction.mul(k)).x),
                        int((Position(*self.position) + direction.mul(k)).y),
                    )
                    if p_c[n][m] != 0:
                        boolean = False
                return boolean
        ...
        if self.name == "rook":
            ...
            if str(pos) in rows and bool(self.case_autorisee(case_souhaitee)):
                ...
                if abs(pos.x - Position(*self.position).x) != 0:
                    direction = vect_unit(case_souhaitee, self.position, "horizontal")
                    for k in range(1, abs(pos.x - Position(*self.position).x)):
                        n, m = (
                            (Position(*self.position).x + direction.mul(k)).x,
                            (Position(*self.position).y + direction.mul(k)).y,
                        )
                    ...
                    direction = vect_unit(case_souhaitee, self.position, "vertical")
        ...
        if self.name == "queen":
            ...
            for i in range(1, 9):
                ...
                if str(pos) in rows_and_diagonals and bool(
                    ...
                    boolean = True
                    if abs(pos.x - Position(*self.position).x) != 0 and abs(pos.y - Position(*self.position).y) == 0:
                        direction = vect_unit(pos, self.position, "horizontal")
                        for k in range(1, abs(pos.x - Position(*self.position).x)):
                            n, m = (
                                int((Position(*self.position) + direction.mul(k)).x),
                                int((Position(*self.position) + direction.mul(k)).y),
                            ...
                            if p_c[n][m] != 0:
                                boolean = False
                                return boolean
                    elif abs(pos.x - Position(*self.position).x) == 0 and abs(pos.y - Position(*self.position).y) != 0:
                        direction = vect_unit(pos, self.position, "vertical")
                        for k in range(1, abs(pos.y - Position(*self.position).y)):
                            n, m = (
                                int((Position(*self.position) + direction.mul(k)).x),
                                int((Position(*self.position) + direction.mul(k)).y),
                            ...
                            if p_c[n][m] != 0:
                                boolean = False
                                return boolean 
                                # Peut-être qu'on peut regrouper les fonctions pour rook, bishop et queen, la seule chose qui change c'est l'ensemble qu'on manipule
                    else : 
                        direction = vect_unit(pos, self.position)
                        for k in range(1, abs(pos.x - Position(*self.position).x)):
                            n, m = (
